...
class MergingIterator implements Cursor<InternalKey,Slice> {
    ...
    final void shift(int i) {
        Cursor<InternalKey,Slice> c = cursor[i];
        if (c.hasNext()) {
            key[i] = c.next().getKey();
        } else {
            key[i] = null;
    ...
    static Cursor<InternalKey,Slice> of(KeyComparator cmp, Cursor<InternalKey,Slice>[] list, int num) {
        if (num < list.length) {
