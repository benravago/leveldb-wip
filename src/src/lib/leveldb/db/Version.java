...
class Version implements Closeable {
    ...
    List<FileMetaData>[] files = repeat(kNumLevels, ArrayList::new); // std::vector<FileMetaData*> files_[config::kNumLevels];
    ...
    public void close() { // Version::~Version() {
        ...
        for (var level = 0; level < kNumLevels; level++) {
            for (var i = 0; i < files[level].size(); i++) {
                var f = files[level].get(i);
                assert (f.refs > 0);
                f.refs--;
                if (f.refs <= 0) {
                    // delete f;
...
// }
    ...
    GetStats get(Slice k, long sequenceNumber, boolean verifyChecksums, boolean fillCache )  { // Get(const ReadOptions&, const LookupKey& key, std::string* val, GetStats* stats);
        var ikey = lookupKey(k,sequenceNumber); // Slice ikey = k.internal_key();
        var userKey = k; // Slice user_key = k.user_key();
        var ucmp = vset.icmp.userComparator;
...
//   FileMetaData* tmp2;
        for (var level = 0; level < kNumLevels; level++) {
            var numFiles = files[level].size();
            if (numFiles == 0) continue;

            // Get the list of files to search in this level
            var listFiles = files[level]; //     FileMetaData* const* files = &files_[level][0];
            if (level == 0) {
                // Level-0 files may overlap each other.
                // Find all files that overlap user_key and
                // process them in order from newest to oldest.
                var tmp = new ArrayList<FileMetaData>(); // tmp.reserve(num_files);
                for (var i = 0; i < numFiles; i++) {
                    var f = listFiles.get(i);
                ...
                var index = findFile(vset.icmp, files[level], ikey);
                if (index >= numFiles) {
                    listFiles = null;
                    numFiles = 0;
                } else {
                    var tmp2 = listFiles.get(index);
            ...
            for (int i = 0; i < numFiles; ++i) {
                ...
                var r = vset.tableCache.get( f.number, f.fileSize, ikey, fillCache );
    ...
    int pickLevelForMemTableOutput(Slice smallestUserKey, Slice largestUserKey) { //   int PickLevelForMemTableOutput(const Slice& smallest_user_key, const Slice& largest_user_key);
        ...
        if (!overlapInLevel(0, smallestUserKey, largestUserKey)) {
            ...
            var start = internalKey(smallestUserKey,kMaxSequenceNumber,kValueTypeForSeek);
            var limit = internalKey(largestUserKey,0,0);
            while (level < kMaxMemCompactLevel) {
                ...
                if (level+2 < kNumLevels) {
                    ...
                    var sum = vset.totalFileSize(overlaps);
    ...
    List<FileMetaData> getOverlappingInputs(int level, InternalKey begin, InternalKey end) {
        ...
        var inputs = new ArrayList<FileMetaData>(); // inputs->clear();
        var userBegin = (begin != null) ? begin.userKey : null;
        var userEnd = (end != null) ? end.userKey : null;
        var userCmp = vset.icmp.userComparator;
        for (var i = 0; i < files[level].size(); ) {
            var f = files[level].get(i++);
            var fileStart = f.smallest.userKey;
            var fileLimit = f.largest.userKey;
