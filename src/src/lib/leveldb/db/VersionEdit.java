...
class VersionEdit {
    ...
    // TODO: make these 3 List's elements simple objects
    // CompactPointer { int, InternalKey }
    // NewFile { int, FileMetaData }
    // DeletedFile { int level, long fileNumber }
    ...
    byte[] encodeTo() {
        ...
        for (var i = 0; i < compactPointers.size(); i++) {
            var compactPointer = compactPointers.get(i);
    ...
    void decodeFrom(byte[] b, int off, int len) {
        clear();
        var input = new ByteDecoder().wrap(b,off,len);
