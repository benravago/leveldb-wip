...
class Versions {
    ...
    static Compaction pickCompaction(VersionSet vset) { // Compaction* VersionSet::PickCompaction() {
        ...
        if (sizeCompaction) {
            ...
            for (var i = 0; i < current.files[level].size(); i++) {
        ...
        if (level == 0) {
            var r = getRange(c.inputs[0], vset.icmp);  // [smallest,largest]
    ...
    static InternalKey[] getRange2(List<FileMetaData> inputs1, List<FileMetaData> inputs2, Comparator<InternalKey> icmp) {
        var inputs = new ArrayList<FileMetaData>( inputs1.size() + inputs2.size() );
  ...
  static class Compaction implements Closeable {
    ...
    void addInputDeletions(VersionEdit edit) {
        for (var which = 0; which < 2; which++) {
            for (var i = 0; i < inputs[which].size(); i++) {
    ...
    boolean isBaseLevelForKey(Slice userKey) {
        ...
        var userCmp = inputVersion.vset.icmp.userComparator;
        for (var lvl = level + 2; lvl < kNumLevels; lvl++) {
            var files = inputVersion.files[lvl];
            while (levelPtrs[lvl] < files.size()) {
                var f = files.get(levelPtrs[lvl]);
    ...
    void setupOtherInputs(VersionSet vset) {
        var current = inputVersion;
        int lvl = level();

        var r = getRange(inputs[0],vset.icmp);
        InternalKey smallest=r[0], largest=r[1];

        inputs[1] = current.getOverlappingInputs(lvl+1, smallest, largest);

        // Get entire range covered by compaction
        var a = getRange2(inputs[0], inputs[1], vset.icmp);
        InternalKey all_start = a[0], all_limit = a[1];

        // See if we can grow the number of inputs in "level" without
        // changing the number of "level+1" files we pick up.
        if (!inputs[1].isEmpty()) {
            var expanded0 = current.getOverlappingInputs(lvl, all_start, all_limit);
            var inputs0_size = vset.totalFileSize(inputs[0]);
            var inputs1_size = vset.totalFileSize(inputs[1]);
            var expanded0_size = vset.totalFileSize(expanded0);
            if (expanded0.size() > inputs[0].size() &&
                inputs1_size + expanded0_size < vset.expandedCompactionByteSizeLimit())
            {
                var n = getRange(expanded0, vset.icmp);
                InternalKey new_start=n[0], new_limit=n[1];
                var expanded1 = current.getOverlappingInputs(lvl+1, new_start, new_limit);
                if (expanded1.size() == inputs[1].size()) {
                    vset.db.info(
                        "Expanding @{0,number} {1,number}+{2,number} ({3,number}+{4,number} bytes)"
                            +  " to {5,number}+{6,number} ({7,number}+{8,number} bytes)",
                        lvl,
                        inputs[0].size(),
                        inputs[1].size(),
        ...
        if (lvl + 2 < kNumLevels) {
            grandparents =
                current.getOverlappingInputs(lvl + 2, all_start, all_limit);
        }
        ...
        // key range next time.
        vset.compactPointer[lvl] = largest;
        edit.setCompactPointer(lvl, largest);
    }
    ...
    Cursor<InternalKey,Slice> makeInputIterator(VersionSet vset) { // Iterator* MakeInputIterator(Compaction* c);
        ...
        assert (num <= space);
        var result = MergingIterator.of(vset.icmp, list, num);
  ...
  static class Builder implements Closeable {
    ...
    final Comparator<FileMetaData> bySmallestKey = new Comparator<FileMetaData>(){
        ...
        public int compare(FileMetaData f1, FileMetaData f2) {
            var r = vset.icmp.compare(f1.smallest,f2.smallest);
            if (r != 0) {
                return r;
            } else {
    ...
    void apply(VersionEdit edit) {
        ...
        for (var i = 0; i < edit.compactPointers.size(); i++) {
            var compactPointer = edit.compactPointers.get(i);
            var level = (int) compactPointer[first];
            vset.compactPointer[level] =
                (InternalKey) compactPointer[second];
        }

        // Delete files
        for (var iter : edit.deletedFiles) {
            var deletedFile = deletedFile(iter);
            var level = (int) deletedFile[first];
            var number = (long) deletedFile[second];
            levels[level].deletedFiles.add(number);
        }

        // Add new files
        for (var i = 0; i < edit.newFiles.size(); i++) {
            var newFile = edit.newFiles.get(i);
            var level = (int) newFile[first];
            var f = (FileMetaData) newFile[second];
            f.refs = 1;
    ...
    void saveTo(Version v) {
        for (var level = 0; level < kNumLevels; level++) {
            ...
            var sortedFiles = new ArrayList<FileMetaData>(baseFiles.size()+addedFiles.size());
