...
/**
 * A Comparator object provides a total ordering
 * across slices that are used as keys in an sstable or a database.
 *
 * A Comparator implementation must be thread-safe
 * since leveldb may invoke its methods concurrently from multiple threads.
 *
 * @param <T>
 */
public interface KeyComparator<T> extends Comparator<T> {
    ...
    /**
     * The underlying byte array comparator.
     * 
     * @return the DB.Comparator implementation;
     */
     ...
     *
     * Used to check for comparator mismatches
     ...
     * and should not be used by any clients of this package.
     *
     * @return the canonical name of this KeyComparator
     */
    String name();

    // Advanced functions:
    //   These are used to reduce the space requirements
