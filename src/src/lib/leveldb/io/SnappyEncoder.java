...
public class SnappyEncoder {
    ...
    public static Slice encode(Slice s) {
        return encode(s.data,s.offset,s.length);
    }

    public static Slice encode(byte[] src, int srcPos, int len) {
    ...
    // System.out.println("s "+srcPos+' '+dstPos+' '+len+' '+' '+src.length+' '+dst.length+' '+src+' '+dst);
